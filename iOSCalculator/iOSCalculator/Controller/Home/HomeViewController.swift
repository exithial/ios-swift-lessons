...
//  iOSCalculator
//
//  Created by Enrique Solís on 28-11-21.
//
...
final class HomeViewController: UIViewController {
    ...
    @IBAction func numberAction(_ sender: UIButton) {
        ...
        if decimal {
            currentTemp = "\(String(describing: currentTemp))\(String(describing: kDecimalSeparator))"
            decimal = false
        }
    ...
    // Verifica si tiene operaciones pendientes y las realiza en caso de que las tenga
    private func operateResult() {
        if operation != .none {
            result()
        }
    }
